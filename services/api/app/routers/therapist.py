...
@router.post("/ai/generate-soap-note")
...
@router.post("/ai/generate-soap-from-audio")
async def generate_soap_from_audio(
    ...
    session_activities: List[str] = None,
    observations: str = "",
    time_in: str = "",
    time_out: str = "",
    units: int = 0,
    ...
    """Generate SOAP note from audio using AI service"""
    try:
        import httpx
        ...
        # Prepare form data
        form_data = {
            "client_age": str(client_age) if client_age else "",
            "diagnosis": diagnosis or "",
            "short_term_goals": ",".join(short_term_goals) if short_term_goals else "",
            "long_term_goals": ",".join(long_term_goals) if long_term_goals else "",
            "session_activities": ",".join(session_activities) if session_activities else "",
            "observations": observations,
            "time_in": time_in,
            "time_out": time_out,
            "units": str(units),
            "treatment_codes": ",".join(treatment_codes) if treatment_codes else ""
        }
        
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{ai_service_url}/generate-soap-from-audio",
                files={"audio_file": ("recording.wav", audio_data, "audio/wav")},
                data=form_data,
                timeout=60.0  # Longer timeout for audio processing
            )
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to generate SOAP from audio: {str(e)}"
        )
